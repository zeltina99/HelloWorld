...
/*
	...
	- 포인터 연산자
		...
		- 배열과 포인터 기본적으로 같다.(배열의 이름은 배열의 첫번째 요소의 주소이다.)
*/
...
/*
	...
	- 스택 영역
		함수가 호출될 때 마다 필요한 변수(지역 변수)가 저장되는 공간
		함수가 끝나면 자동으로 정리
*/
	...
	- 구조체 데이터 접근법
		...
		- 구조체의 생성자
			...
			- 객체
				...
				Enemy* pEnemy = new Enemy();	// Enemy객체 하나가 만들어짐
			- 멤버 변수 초기화에 사용(값을 계산하거나 변경해서 넣을 때 좋음)
			- 일반 함수와 다른 특징
				1. 이름이 구조체 이름과 같다.
				2. 리턴 타입이 없다.
...
/*
	...
	- 구성 요소
		...
		- 멤버 변수 : 객체의 상태나 데이터를 저장하는 변수. (속성, 필드라고도 함)
		- 멤버 함수 : 객체가 할 수 있는 동작이나 기능을 정의한 함수. (메서드라고도 함)
*/

/*
	접근 제한자(Access Modifier)
	- 이 객체의 내부(멤버)를 누구까지 볼 수 있을 것인가를 설정
...
/*
	상속
	- 부모 클래스의 멤버를 물려 받는 행위
	- 특징
	 - 코드 재사용성이 증가한다.
	 - 계층구조로 객체들의 관ㄱ{를 명확히 할 수 있다.
...
/*
	인터페이스
	 ...
	 - (클래스가 반드시 구현해야 할 멤버 함수들의 명세를 정의하기 위해 사용)
	 - 주의 사항
		- 이름은 무조건 대문자 I로 시작하는 것이 관례(필수적인 관례)
		- 멤버는 순수 가상 함수만 있다.(소멸자 제외)
*/
...
/*
	enum class
	 ...
	 - enum class : 더 안전해진 enum.(안전해진만큼 불편해진 부분도 있음)
		- enum class는 코드의 명확성과 안정성을 향상 시킨다.
...
/*
	빅오 표기법
		...
		- O(n) : 알고리즘을 돌릴 때 데이터의 개수의 비례해서 속도가 느려진다.
...
/*
	트리
	 - 특징
		...
		- 하나의 Root(뿌리)에서 시작하여 가지가 뻗어 나가는 형태
		- 순환이 없는 그래프의 한 종류
		- 대표적인 비선형 자료구조
	 - 장점
...
/*
	std::vector
	 - 특징
		- 동적 배열(자동으로 크기가 조절됨)
		- 연속된 메모리를 가진다.(배열의 특성) -> 캐시효율 좋음, 랜덤 엑세스 성능 좋음( O(1) ), 마지막에 추가 삭제할 때 매우 효율적
	 - 사용처
	...
	람다(Lambda)
	 ...
	 - 기본구조
		...
		캡쳐 : 람다 함수 외부의 변수에 어떻게 접근할지 지정
			- [] : 외부 변수를 사용하지 않는다.
				- 가능하면 사용하지 말 것(성능에 영향을 준다)
				- [&] : 외부의 모든 변수를 참조로 캡쳐한다.(값 변경 불가능)
				- [=] : 외부의 모든 변수를 복사로 캡쳐한다.(값 변경 불가능)
				- [this] : 클래스 멤버 변수와 멤버 함수에 접근 할 수 있다.
			- [변수명] : 특정 변수만 복사로 캡쳐
			- [&변수명] : 특정 변수만 참조로 캡쳐
			- [변수명1, &변수명2] : 여러변수를 복사/참조로 캡쳐
		파라메터 : 일반 함수처럼 람자함수가 받을 파라메터들을 정의
